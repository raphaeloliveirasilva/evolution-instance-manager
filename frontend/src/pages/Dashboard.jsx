...
// Note que removemos o import da Sidebar e SettingsContent (se não usado)
...
export default function Dashboard() {
  ...
  // --- 1. ESTADO DA ABA ---
  const [activeTab, setActiveTab] = useState(location.state?.targetTab || 'instances');

  // --- 2. EFEITO DE NAVEGAÇÃO ---
  useEffect(() => {
  ...
  // --- 4. ESTADOS DE UI (Modais apenas) ---
  // Removidos: isSidebarOpen, isCollapsed (Layout cuida disso agora)
  ...
  async function handleSaveUser(e) {
    ...
    try {
      ...
      setEditingUser(null);
      loadUsers();
    } catch (err) { alert(err.response?.data?.error || "Erro ao salvar usuário"); }
  }
  ...
  // Removido handleLogout pois agora é responsabilidade da Sidebar no Layout

  // --- RENDERIZAÇÃO LIMPA ---
  // Não há mais div wrapper, main ou sidebar aqui. Apenas o conteúdo.
  return (
    <>
      {activeTab === 'instances' && (
        <InstancesContent
      ...
      {/* MODAIS (Sempre presentes no DOM, mas ocultos) */}
      <PlanModal
